...
void QuadTree::subdivideNode(QuadTreeNode* node) {
	...
	node->children[0] = new QuadTreeNode(QuadTreePoint(node->position.x - subWidth, node->position.y - subHeight), subWidth, subHeight, node);
	node->children[1] = new QuadTreeNode(QuadTreePoint(node->position.x + subWidth, node->position.y - subHeight), subWidth, subHeight, node);
	node->children[2] = new QuadTreeNode(QuadTreePoint(node->position.x - subWidth, node->position.y + subHeight), subWidth, subHeight, node);
	node->children[3] = new QuadTreeNode(QuadTreePoint(node->position.x + subWidth, node->position.y + subHeight), subWidth, subHeight, node);
}
...
void QuadTree::solveCollisionsInNode(QuadTreeNode *node, void ( *collideParticles) (Particle *p1, Particle *p2)) {
	...
	if (i == 0) {
		...
		// after collision need to update all cells
		// std::vector secondaryVec = vec; // copy so that things dont get inserted in an infinite loop
		// for (i = size - 1; i > static_cast<unsigned int>(0 - 1); i--) {
		// 	p1 = secondaryVec[i];
		// 	secondaryVec.pop_back();
		// 	insert(p1);
		// }
	}
...
void QuadTree::checkCollisionsInNeighboringQuadrants(QuadTreeNode *node, void ( *collideParticles) (Particle *p1, Particle *p2)) {
	...
	// 	QuadTreeNode* neNode = parent->children[1];
	// 	QuadTreeNode* swNode = parent->children[2];
	// 	QuadTreeNode* seNode = parent->children[3];
