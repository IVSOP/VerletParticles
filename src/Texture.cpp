...
void Texture::Bind (unsigned int slot) const {
	GLCall(glActiveTexture(GL_TEXTURE0 + slot)); //slot 0
	GLCall(glBindTexture(GL_TEXTURE_2D, m_RendererID));
}
...
// slot is already offset by GL_TEXTURE0, goes up to 31!!!!!!
// id gets set to the texture ID
void makeTexture(GLuint *id, const char path[], const GLuint slot) {
	stbi_set_flip_vertically_on_load(1);
	int width, height, BPP;
	unsigned char *buffer =	stbi_load(path, &width, &height, &BPP, 4); // 4 -> RGBA
...
void bindTexture(GLuint id, const GLuint slot) {
	GLCall(glActiveTexture(GL_TEXTURE0 + slot));
	GLCall(glBindTexture(GL_TEXTURE_2D, id));
}
